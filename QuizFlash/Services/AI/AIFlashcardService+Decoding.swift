...
private enum AIRawFormalNotationRepair {
    static let rawFormalScalars = Set("φψτΓΔΣΠΛΩΦΨ∀∃∈∉⊆⊂⊇⊃∅∧∨¬→↔⇒⇔⊢⊨⊥⊤□ᶜ₀₁₂₃₄₅₆₇₈₉⁰¹²³⁴⁵⁶⁷⁸⁹".unicodeScalars)

    static let naturalLanguageBoundaryWords: Set<String> = [
        "a", "al", "ale", "are", "as", "ca", "care", "cu", "daca", "dacă", "de", "din", "este",
        "fie", "in", "în", "lui", "notata", "notată", "o", "orice", "pentru", "prin", "sau",
        "se", "si", "și", "un", "unde", "unei", "valoarea",
        "and", "as", "for", "if", "in", "is", "of", "or", "the", "then", "to", "where"
    ]
}
...
extension AIFlashcardService {
    ...
    func sanitizedZoneStrings(_ values: [String]) -> [String] {
        values
            .map { value in
                ...
                guard !isFencedCodeBlock(trimmed) else { return trimmed }
                return repairRawFormalNotation(in: AIZoneParser.sanitizeLatex(trimmed))
            }
            .filter { !$0.isEmpty }
    }

    func repairRawFormalNotation(in value: String) -> String {
        guard containsRawFormalNotation(value) else { return value }
        ...
        while cursor < value.endIndex {
            if value[cursor] == "`" {
                let end = value[value.index(after: cursor)...].firstIndex(of: "`") ?? value.index(before: value.endIndex)
                result += value[cursor...end]
                cursor = value.index(after: end)
                continue
            }

            if value[cursor] == "$" {
                let delimiter = value[value.index(after: cursor)..<value.endIndex].first == "$" ? "$$" : "$"
                let contentStart = value.index(cursor, offsetBy: delimiter.count)
                if let endRange = value.range(of: delimiter, range: contentStart..<value.endIndex) {
                    result += value[cursor..<endRange.upperBound]
                    cursor = endRange.upperBound
                    continue
                }
            }

            let nextProtected = value[cursor...].firstIndex { $0 == "`" || $0 == "$" } ?? value.endIndex
            result += repairPlainRawFormalNotation(String(value[cursor..<nextProtected]))
            cursor = nextProtected
        }

        return result
    }

    private func repairPlainRawFormalNotation(_ value: String) -> String {
        let tokens = formalRepairTokens(from: value)
        ...
        while index < tokens.count {
            ...
            guard !token.isWhitespace, isFormalCandidateToken(token.text) else {
                output += token.text
                index += 1
                continue
            }
            ...
            var end = index
            var hasCoreFormal = false
            var collected = ""
            while end < tokens.count {
                let current = tokens[end]
                if current.isWhitespace {
                    let next = end + 1
                    guard next < tokens.count, isFormalCandidateToken(tokens[next].text) else { break }
                    collected += current.text
                    end += 1
                    continue
                }

                guard isFormalCandidateToken(current.text) else { break }
                hasCoreFormal = hasCoreFormal || containsCoreFormalNotation(current.text)
                collected += current.text
                end += 1
            }

            if hasCoreFormal, let repaired = latexWrappedFormalSpan(from: collected) {
                output += repaired
                index = end
            } else {
                output += token.text
    ...
    private func formalRepairTokens(from value: String) -> [(text: String, isWhitespace: Bool)] {
        ...
        for character in value {
            ...
            if character == ";" {
                if !current.isEmpty {
                    tokens.append((current, currentWhitespace ?? false))
                    current = ""
                }
                tokens.append((String(character), false))
                currentWhitespace = nil
                continue
            }

            if currentWhitespace == nil {
    ...
    private func containsRawFormalNotation(_ value: String) -> Bool {
        value.unicodeScalars.contains { scalar in
            AIRawFormalNotationRepair.rawFormalScalars.contains(scalar)
        }
    }

    private func isFormalCandidateToken(_ token: String) -> Bool {
        let trimmed = token.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return false }
        guard !trimmed.contains(AIZoneParser.zoneDelimiter) else { return false }
        if trimmed == ";" || trimmed == "." || trimmed == "?" || trimmed == "!" { return false }
        if containsCoreFormalNotation(trimmed) { return true }
        ...
        if stripped == stripped.lowercased(), AIRawFormalNotationRepair.naturalLanguageBoundaryWords.contains(stripped) { return false }
        if stripped.count > 8, !stripped.unicodeScalars.allSatisfy({ CharacterSet.uppercaseLetters.contains($0) || CharacterSet.decimalDigits.contains($0) }) {
            return false
        }
    ...
    private func containsCoreFormalNotation(_ token: String) -> Bool {
        if token.unicodeScalars.contains(where: { AIRawFormalNotationRepair.rawFormalScalars.contains($0) }) {
            return true
        }
    ...
    private func latexWrappedFormalSpan(from span: String) -> String? {
        let leadingWhitespace = span.prefix { $0.isWhitespace }
        let trailingWhitespace = String(span.reversed().prefix { $0.isWhitespace }.reversed())
        var core = span.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        let latex = latexNormalizedFormalSpan(core)
        guard !latex.isEmpty else { return nil }
        return "\(leadingWhitespace)$\(latex)$\(trailingPunctuation)\(trailingWhitespace)"
    }

    private func latexNormalizedFormalSpan(_ span: String) -> String {
        var value = span
            .replacingOccurrences(of: "**", with: "")
            .replacingOccurrences(of: "{", with: #"\\{"#)
            .replacingOccurrences(of: "}", with: #"\\}"#)
        ...
        for (raw, latex) in replacements {
            value = value.replacingOccurrences(of: raw, with: latex)
        }

        value = applyRegex(#"\\(varphi|psi|tau|Gamma|Delta|Sigma|Pi|Lambda|Omega|Phi|Psi)([0-9]+)"#, to: value, template: #"\\$1_{$2}"#)
        value = applyRegex(#"\^([A-Za-z0-9]+)"#, to: value, template: #"^{$1}"#)
        value = applyRegex(#"_([A-Za-z0-9]+)"#, to: value, template: #"_{$1}"#)
        value = value.replacingOccurrences(of: "\\\\", with: "\\")
        return value
    }

    private func applyRegex(_ pattern: String, to value: String, template: String) -> String {
