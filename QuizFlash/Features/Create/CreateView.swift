        ...
        ZStack {
            ...
            if viewModel.showAIOptionsOverlay {
                AIOptionsOverlay(
                    ...
                    useFastOCR: $viewModel.useFastOCR, // <-- Aici era eroarea
                    onGenerate: {
...
struct AIOptionsOverlay: View {
    ...
    @Binding var useFastOCR: Bool // NOU: Legătură către ViewModel
    var onGenerate: () -> Void
    var onCancel: () -> Void
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 24) {
                ...
                // NOU: Toggle-ul pentru OCR rapid
                VStack(alignment: .leading, spacing: 6) {
                    Toggle("Extragere Text Rapidă", isOn: $useFastOCR)
                        .font(.subheadline.weight(.semibold))
                        .tint(.accentColor)

                    Text(useFastOCR ? "Folosește OCR local. Procesare în doar câteva secunde. Ideal pentru documente cu mult text." : "Trimite imaginile către AI. Durează mai mult, dar înțelege perfect schemele, diagramele și layout-ul complex.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
