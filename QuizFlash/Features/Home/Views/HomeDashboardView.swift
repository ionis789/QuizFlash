        ...
        ZStack {
            if correctCount > 0 {
                outcomeSegment(
                    ...
                    to: correctFraction,
                    color: correctColor,
                    lineCap: retryCount > 0 ? .butt : .round
                )
            }

            if retryCount > 0 {
                outcomeSegment(
                    from: correctFraction,
                    to: min(correctFraction + retryFraction, 1),
                    color: retryColor,
                    lineCap: .round
                )
            }

            if correctCount > 0 && retryCount > 0 {
                outcomeSegment(
                    from: max(correctFraction - greenCapOverlayFraction, 0),
                    to: correctFraction,
                    color: correctColor,
                    lineCap: .round
                )
...
private struct HomeDashboardFolderCard: View {
    ...
    let usesRegularMetrics: Bool
    let actionFeedback: HomeFolderActionFeedback?
    let onOpen: () -> Void
