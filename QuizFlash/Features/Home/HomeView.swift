...
// Responsible for:
//   - Injecting SwiftData queries into child views
//   - Computing scroll geometry (extendedHeight, scrollDistance)
//   - Orchestrating CalendarSectionView and HomeDashboardView
//
...
struct HomeView: View {
    ...
    var body: some View {
        ...
        .background(Color(uiColor: .systemBackground).ignoresSafeArea())
            ...
            .onAppear {
            calendarVM.setupIfNeeded()
            // updateLogsCache is already driven by onChange(of: dailyLogs).
            // Calling it here only if cache is actually empty prevents a forced
            // re-render cycle on every tab return that resets child @State vars.
