...
//    This binding is gated on `isTabBarVisible` so that phantom UIKit tab events
//    fired by the hidden native UITabBar cannot trigger `popToRoot()`.
//
...
struct MainAppView: View {
    ...
    ///
    /// The `guard isTabBarVisible` gate is critical: UITabBar.appearance().isHidden
    /// hides the native bar visually but leaves its hit-testing active. Without
    /// this guard, `_tabBarItemClicked:` events generated by the hidden native bar
    /// would call `popToRoot()` — resetting navigation state (including selection
    /// mode) when the user taps anywhere near the bottom of the screen.
    private var tabSelectionBinding: Binding<AppTabBar> {
        Binding(
            get: { router.activeTab },
            set: { tappedTab in
                // Block all native UITabBar events when the custom bar is not
                // visible. The hidden UITabBar still receives taps and would
                // otherwise fire popToRoot() or switch tabs unexpectedly.
