...
final class AICardJSONDecodingTests: XCTestCase {
    ...
    func testPromptUsesCanonicalCardDTOSchema() {
        ...
        XCTAssertTrue(prompt.contains("render it through LaTeX math instead of mixing raw native glyphs with rendered formulas"))
        XCTAssertTrue(prompt.contains("Keep notation style consistent across all cards in the same response"))
        XCTAssertTrue(prompt.contains("Apply the same notation rules to every card surface"))
