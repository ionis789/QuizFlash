...
Retain every useful diagnostic introduced during QuizFlash debugging as a reusable developer tool. It `MUST` be disabled by default, have a persistent switch in the development-only Settings diagnostics surface, and expose copy/clear actions when it records a report or timeline. If it observes startup or another early lifecycle phase, label that enabling it requires a relaunch. Sanitize sensitive values instead of removing the diagnostic. Developer UI, preference reads/writes, probes, watchers, trace collection, debug tasks, mock behavior, and their call-site argument evaluation `MUST` be excluded at compile time from Release builds; a runtime false check or Release no-op implementation is not sufficient. Production-safe operational error logging is separate from developer diagnostics and must remain privacy-redacted.
