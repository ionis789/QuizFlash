...
- The App Store Connect API credential validates in RevenueCat. Debug/Test Store loads the monthly package and localized 9.99 USD test price on a physical device. Purchase/restore, RevenueCat entitlement activation, Worker reconciliation, Firestore premium projection, and Premium quota activation are confirmed end to end for the sandbox Firebase account. A physical-device check with the Apple RevenueCat SDK key previously reached RevenueCat but StoreKit returned no App Store product; complete the subscription review screenshot, wait for banking and DSA trader verification to finish, recheck the In-App Purchase key, then allow Apple sandbox propagation before retrying the Release path.
...
4. Configure and verify the remote RevenueCat webhook credentials and delivery, then validate purchase, renewal, cancellation, restore, and cross-device refresh on the Apple sandbox Release path.
